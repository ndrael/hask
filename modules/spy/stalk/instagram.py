...
def run(username):
    ...
    try:
        ...
        try:
            ...
            print(f"{Y}[~] Instagram API blocked or changed. Try again later.{N}")
            return
        ...
        print(f"{G}[+] External  : {user.get('external_url', 'N/A')}{N}")
        print(f"{G}[+] Verified  : {user.get('is_verified', False)}{N}")
        print(f"{G}[+] Private   : {user.get('is_private', False)}{N}")
        print(f"{G}[+] Business  : {user.get('is_business_account', False)}{N}")
